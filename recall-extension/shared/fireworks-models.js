...
export function fillModelSelect(selectEl, models, currentId, fallbackId) {
  ...
  if (selected && known) {
    ...
    selectEl.value = fallbackId || models[0]?.id || CUSTOM_MODEL_VALUE;
  }
}

export function resolveSelectedModel(selectEl, customInputEl, fallbackId) {
  if (!selectEl) return fallbackId || '';

  if (selectEl.value === CUSTOM_MODEL_VALUE) {
    const custom = customInputEl?.value?.trim() || '';
    return custom || fallbackId || '';
  }

  return selectEl.value || fallbackId || '';
}

export function syncCustomModelVisibility(selectEl, customInputEl, currentId) {
  if (!selectEl || !customInputEl) return;

  const isCustom = selectEl.value === CUSTOM_MODEL_VALUE;
  customInputEl.hidden = !isCustom;
  customInputEl.required = isCustom;

  if (isCustom && currentId && !customInputEl.value) {
    customInputEl.value = currentId;
  }
}
