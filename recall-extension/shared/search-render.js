...
export function safeAnchor(url, text, extraClass = '') {
  ...
  return `<a href="${escapeHtml(href)}"${classAttr} target="_blank" rel="noopener noreferrer">${label}</a>`;
}
...
export function bindSearchCards(container, { onOpen, onDelete } = {}) {
  container.querySelectorAll('.search-card').forEach((card) => {
    card.addEventListener('click', (event) => {
      ...
      const url = safeHref(card.dataset.url);
      if (!url) {
        return;
      }
      if (onOpen) {
