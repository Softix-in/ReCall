...
function bindEvents() {
  ...
  $('#quick-save-btn').addEventListener('click', async () => {
    ...
    const allowed = await ensureCaptureHostPermission();
    if (!allowed) {
      $('#quick-save-btn').disabled = false;
      showToast('Grant site access to save pages.', 'error');
      return;
    }
  ...
  $('#extract-knowledge-btn').addEventListener('click', async () => {
    ...
    if (!allowed) {
      ...
      showToast('Grant site access to extract this page.', 'error');
      return;
    }
  ...
  $('#save-note-btn').addEventListener('click', async () => {
    ...
    const allowed = await ensureCaptureHostPermission();
    if (!allowed) {
      showToast('Grant site access to save this page.', 'error');
      return;
    }
