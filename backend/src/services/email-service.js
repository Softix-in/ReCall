...
async function sendMail({ to, subject, text, html }) {
  ...
  if (!transport) {
    ...
    console.warn('[email] SMTP is not configured; auth email was not sent');
    return { sent: false };
  }
...
async function sendVerificationEmail(email, token) {
  ...
  return sendMail({
    ...
    html: `<p>Welcome to Recall!</p><p><a href="${escapeHtml(url)}">Verify your email</a></p><p>This link expires in 24 hours.</p>`,
  });
}
...
async function sendPasswordResetEmail(email, token) {
  ...
  return sendMail({
    ...
    html: `<p><a href="${escapeHtml(url)}">Reset your password</a></p><p>This link expires in 1 hour.</p>`,
  });
}
...
async function sendEmailChangeEmail(newEmail, token) {
  ...
  return sendMail({
    ...
    html: `<p><a href="${escapeHtml(url)}">Confirm your new email</a></p><p>This link expires in 24 hours.</p>`,
  });
}
