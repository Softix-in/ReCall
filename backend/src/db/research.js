...
const { withUserContext, query, formatVector } = require('./pg-pool');
...
async function getCompanyInternal(companyId) {
  const result = await query('SELECT * FROM research_companies WHERE id = $1', [companyId]);
  return rowToCompany(result.rows[0]);
}
...
async function getJobInternal(jobId) {
  const result = await query('SELECT * FROM research_jobs WHERE id = $1', [jobId]);
  return rowToJob(result.rows[0]);
}
