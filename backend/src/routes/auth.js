...
function createAuthRouter() {
  ...
  router.get('/verify-email', async (req, res) => {
    ...
    try {
      ...
      res.status(error.status || 400).send(htmlPage('Verification failed', `<p class="error">${error.message}</p>`));
    }
  });
  ...
  router.post('/reset-password', express.urlencoded({ extended: false }), async (req, res) => {
    ...
    try {
      ...
      if (req.accepts('html')) {
        res.status(error.status || 400).send(htmlPage('Reset failed', `<p class="error">${error.message}</p>`));
        return;
      }
  ...
  router.get('/confirm-email-change', async (req, res) => {
    ...
    try {
      ...
      res.send(htmlPage(
        'Email updated',
        `<p class="success">Your email is now <strong>${user.email}</strong>. Sign in again in the Recall extension if needed.</p>`,
      ));
    } catch (error) {
      res.status(error.status || 400).send(htmlPage('Confirmation failed', `<p class="error">${error.message}</p>`));
    }
  });
