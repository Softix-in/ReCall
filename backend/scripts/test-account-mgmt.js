 ...
 * Requires a running backend. Auth links are never returned in API JSON; tokens are
 * inserted directly into auth_tokens for verification.
 */
...
async function insertAuthToken(userId, type, payload = null, ttlMs = 60 * 60 * 1000) {
  const token = crypto.randomBytes(32).toString('base64url');
  const now = Date.now();
  await query(
    `INSERT INTO auth_tokens (user_id, token_hash, type, payload, expires_at, created_at)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [userId, hashToken(token), type, payload ? JSON.stringify(payload) : null, now + ttlMs, now],
  );
  return token;
}
...
async function main() {
  ...
  assert(health.body?.ok === true, 'Health should return { ok: true }');
  ...
  assert(!registered.body.verification_url, 'API must not return verification_url');
  ...
  const verifyToken = await insertAuthToken(registered.body.user.id, 'email_verify');
  const verified = await request('POST', '/auth/verify-email', { body: { token: verifyToken } });
  assert(verified.status === 200, `Verify email failed: ${verified.status} ${JSON.stringify(verified.body)}`);
  ...
  assert(!changeEmail.body.confirm_url, 'API must not return confirm_url');
  console.log('Change email request succeeded');

  const confirmToken = await insertAuthToken(registered.body.user.id, 'email_change', { new_email: newEmail });
  ...
  assert(!forgot.body.reset_url, 'API must not return reset_url');
  console.log('Forgot password request succeeded');

  const userRow = await query('SELECT id FROM users WHERE email = $1', [newEmail]);
  const resetToken = await insertAuthToken(userRow.rows[0].id, 'password_reset');
